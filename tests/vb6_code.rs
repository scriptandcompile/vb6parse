...
fn randomize_effects_project_load() {
    let project_file_bytes = include_bytes!("./data/vb6-code/Randomize-effects/RandomizationFX.vbp");
    ...
    assert_eq!(project.help_file_path, Some(b"".as_bstr()));
    assert_eq!(project.title, Some(b"Emboss / Engrave Application".as_bstr()));
    assert_eq!(
        project.exe_32_file_name,
    ...
    assert_eq!(project.version_info.file_description, Some(b"".as_bstr()));
    assert_eq!(project.version_info.copyright, Some(b"Published in 2010 by Tanner Helland".as_bstr()));
    assert_eq!(project.version_info.trademark, Some(B("").as_bstr()));
    assert_eq!(project.version_info.product_name, Some(B("").as_bstr()));
}
...
fn screen_capture_project_load() {
    ...
    assert_eq!(project.help_file_path, Some(b"".as_bstr()));
    assert_eq!(project.title, Some(b"Screen Capture Demonstration".as_bstr()));
    assert_eq!(
        project.exe_32_file_name,
