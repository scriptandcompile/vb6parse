...
fn expression_whitespace_conservation() {
    ...
    let has_whitespace = assignment.children.iter().any(|child| 
        child.kind == SyntaxKind::Whitespace
    );
    
    assert!(has_whitespace, "AssignmentStatement should contain whitespace tokens");
    
    // Check specifically for the whitespace around the equals sign
    let children = &assignment.children;
    let equal_pos = children.iter().position(|c| c.kind == SyntaxKind::EqualityOperator)
        .expect("Should find EqualityOperator");
        
    assert!(equal_pos > 0, "EqualityOperator should not be the first child");
    assert!(children[equal_pos - 1].kind == SyntaxKind::Whitespace, "Should have whitespace before =");
    assert!(children[equal_pos + 1].kind == SyntaxKind::Whitespace, "Should have whitespace after =");
    ...
    if let Some(expr) = expression {
        ...
        let plus_pos = expr.children.iter().position(|c| c.kind == SyntaxKind::AdditionOperator);
        
        if let Some(pos) = plus_pos {
            assert!(expr.children[pos - 1].kind == SyntaxKind::Whitespace, "Should have whitespace before +");
            assert!(expr.children[pos + 1].kind == SyntaxKind::Whitespace, "Should have whitespace after +");
        }
    } else {
    ...
    assert_eq!(cst.text(), input, "Full text reconstruction should match input exactly");
}
...
fn find_node_by_kind<'a>(node: &'a vb6parse::parsers::cst::CstNode, kind: SyntaxKind) -> Option<&'a vb6parse::parsers::cst::CstNode> {
    if node.kind == kind {
        return Some(node);
    }
